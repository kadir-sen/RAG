  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        <Link to="/chronology" className="font-mono text-[10px] uppercase text-[var(--text-muted)]">← All chronologies</Link>
        <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          {job.status === 'ready' && <button type="button" onClick={() => void downloadReport(job)} className="shrink-0 bg-[var(--accent)] px-5 py-3 font-mono text-[10px] uppercase tracking-[.12em] text-[var(--accent-ink)]">Download Word ↓</button>}
        </div>
        ...
        {job.status === 'ready' && <div className="mt-8 grid lg:grid-cols-[1fr_280px] gap-6">
          ...
          <aside><p className="font-mono text-[10px] uppercase tracking-[.14em] text-[var(--text-muted)]">Verified sources · {evidence.length}</p><div className="mt-3 space-y-2">{evidence.map((item) => <button type="button" key={String(item.source_id)} onClick={() => void showSource(String(item.source_id))} className="w-full border border-[var(--border)] bg-[var(--wash)] p-3 text-left"><span className="block truncate text-[11px] text-[var(--text-primary)]">{String(item.file_name || item.title || '')}</span><span className="mt-1 block font-mono text-[9px] text-[var(--text-muted)]">{String(item.source_id)}{item.page ? ` · p.${item.page}` : ''}</span></button>)}</div></aside>
        </div>}

        {source && <section className="fixed inset-x-4 bottom-4 z-40 mx-auto max-w-3xl border border-[var(--ink)] bg-[var(--bg-primary)] p-4 shadow-xl"><div className="flex justify-between gap-4"><div><p className="text-[12px] font-semibold text-[var(--text-primary)]">{String(source.source.file_name || source.record.file_name || 'Project source')}</p><p className="mt-2 max-h-32 overflow-auto text-[11px] leading-5 text-[var(--text-secondary)]">{String(source.source.excerpt || '')}</p></div><button type="button" onClick={() => setSource(null)} className="self-start text-[var(--text-muted)]" aria-label="Close source preview">×</button></div>{source.record.doc_id && <button type="button" onClick={() => openDocument({ docId: source.record.doc_id || '', fileName: source.record.file_name || String(source.source.file_name || ''), anchor: source.source.page ? `page_${source.source.page}` : undefined, highlightText: String(source.source.excerpt || '') })} className="mt-3 text-[10px] text-[var(--accent)]">Open project document →</button>}</section>}
