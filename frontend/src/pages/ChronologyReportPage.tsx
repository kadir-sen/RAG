  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        {source && <section className="fixed inset-x-4 bottom-4 z-40 mx-auto max-w-3xl border border-[var(--ink)] bg-[var(--bg-primary)] p-4 shadow-xl"><div className="flex justify-between gap-4"><div><p className="text-[12px] font-semibold text-[var(--text-primary)]">{String(source.source.file_name || source.record.file_name || 'Project source')}</p><p className="mt-2 max-h-32 overflow-auto text-[11px] leading-5 text-[var(--text-secondary)]">{String(source.source.excerpt || '')}</p></div><button type="button" onClick={() => setSource(null)} className="self-start text-[var(--text-muted)]" aria-label="Close source preview">×</button></div>{source.record.doc_id && <button type="button" onClick={() => openDocument({ docId: source.record.doc_id || '', fileName: source.record.file_name || String(source.source.file_name || ''), anchor: sourceAnchor(source.source), highlightText: String(source.source.excerpt || '') })} className="mt-3 text-[10px] text-[var(--accent)]">Open project document →</button>}</section>}
