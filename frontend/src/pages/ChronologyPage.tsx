        ...
        <section className="mt-7 border border-[var(--border)] bg-[var(--wash)] p-5 md:p-7">
          <form onSubmit={submit} className="grid md:grid-cols-2 gap-3">
            <label className="md:col-span-2 text-[11px] text-[var(--text-secondary)]">Topic or research question
              <textarea required minLength={3} value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Describe the issue, event or subject to investigate…" className="mt-2 block w-full min-h-28 border border-[var(--border)] bg-[var(--bg-primary)] p-3 text-[14px] text-[var(--text-primary)]" />
            </label>
            ...
            <div className="md:col-span-2 flex items-center justify-between gap-4 pt-2">
              <p className="text-[10px] text-[var(--text-muted)]">The report number is assigned automatically.</p>
              <button disabled={submitting || !topic.trim() || project?.role === 'viewer'} className="px-6 py-3 bg-[var(--accent)] text-[var(--accent-ink)] font-mono text-[10px] uppercase tracking-[.14em] disabled:opacity-40">{submitting ? 'Queuing…' : 'Generate chronology →'}</button>
            </div>
          </form>
