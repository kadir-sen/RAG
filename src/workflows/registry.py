...
WORKFLOWS: Dict[WorkflowId, WorkflowSpec] = {
    ...
    WorkflowId.PROGRAMME_CRITICAL_PATH: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_CRITICAL_PATH,
        name="Baseline critical path",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Show the baseline critical path.",
                          "What's on the longest path?",
                          "Trace the driving activities."],
        required_inputs=["a programme .xer file"],
        steps=[_step("resolve", "resolve.inputs", "Resolve the programme XER"),
               _step("path", "programme.critical_path",
                     "Longest-path trace (float fallback)",
                     expected_outputs=["data_table", "artifact_link"])],
        expected_blocks=["markdown_text", "data_table", "artifact_link",
                         "caveats", "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_critical_path",
    ),
    WorkflowId.PROGRAMME_COMPARISON: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_COMPARISON,
        name="Programme revision comparison",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Compare the two programme revisions.",
                          "What changed between the updates?",
                          "Claim-digger change log."],
        required_inputs=["two dated .xer revisions"],
        steps=[_step("resolve", "resolve.inputs", "Earlier + later XER"),
               _step("compare", "programme.comparison",
                     "Diff activities, logic and retrospective actuals",
                     expected_outputs=["data_table"])],
        expected_blocks=["markdown_text", "data_table", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_comparison",
    ),
    WorkflowId.PROGRAMME_PROGRESS: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_PROGRESS,
        name="Progress S-curve",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Show the progress S-curve.",
                          "Planned vs actual progress.",
                          "How far behind plan are we?"],
        required_inputs=["a baseline and a later dated .xer revision"],
        steps=[_step("resolve", "resolve.inputs", "Baseline + updates"),
               _step("progress", "programme.progress",
                     "Planned vs recorded cumulative progress",
                     expected_outputs=["chart", "data_table"])],
        expected_blocks=["markdown_text", "data_table", "chart", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_progress",
    ),
    WorkflowId.PROGRAMME_WINDOWS: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_WINDOWS,
        name="Windows / period movement",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Run a windows analysis.",
                          "Period movement across the updates.",
                          "Window-by-window delay analysis."],
        required_inputs=["two or more dated .xer revisions"],
        steps=[_step("resolve", "resolve.inputs", "Order dated revisions"),
               _step("windows", "programme.windows",
                     "Per-window movement + critical-path turnover",
                     expected_outputs=["data_table", "chart"])],
        expected_blocks=["markdown_text", "data_table", "chart", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_windows",
    ),
    WorkflowId.PROGRAMME_FLOAT_EROSION: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_FLOAT_EROSION,
        name="Float erosion tracker",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Track float erosion.",
                          "Which activities are losing float?",
                          "Total float trend across revisions."],
        required_inputs=["two or more dated .xer revisions"],
        steps=[_step("resolve", "resolve.inputs", "Order dated revisions"),
               _step("erosion", "programme.float_erosion",
                     "Per-window float consumption",
                     expected_outputs=["data_table", "chart"])],
        expected_blocks=["markdown_text", "data_table", "chart", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_float_erosion",
    ),
