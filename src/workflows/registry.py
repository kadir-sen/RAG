...
WORKFLOWS: Dict[WorkflowId, WorkflowSpec] = {
    ...
    WorkflowId.PROGRAMME_RESOURCES: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_RESOURCES,
        name="Planned resource histogram",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Show the resource histogram.",
                          "Planned manpower loading.",
                          "Resource loading profile."],
        required_inputs=["a resourced programme .xer"],
        steps=[_step("resolve", "resolve.inputs", "Resolve the programme XER"),
               _step("resources", "programme.resources",
                     "Monthly planned resource loading",
                     expected_outputs=["data_table", "chart"])],
        expected_blocks=["markdown_text", "data_table", "chart", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_resources",
    ),
    WorkflowId.PROGRAMME_ASBUILT_PATH: WorkflowSpec(
        workflow_id=WorkflowId.PROGRAMME_ASBUILT_PATH,
        name="As-built critical path",
        status=WorkflowStatus.AVAILABLE,
        trigger_examples=["Reconstruct the as-built critical path.",
                          "Observational as-built analysis.",
                          "Criticality persistence across revisions."],
        required_inputs=["two or more successive dated .xer revisions"],
        steps=[_step("resolve", "resolve.inputs", "Order dated revisions"),
               _step("asbuilt", "programme.asbuilt_path",
                     "Stitched driving path + persistence index",
                     expected_outputs=["data_table"])],
        expected_blocks=["markdown_text", "data_table", "caveats",
                         "validation_status"],
        risk="high",
        analyst_review_required=True,
        target="adapter:programme_asbuilt_path",
    ),
