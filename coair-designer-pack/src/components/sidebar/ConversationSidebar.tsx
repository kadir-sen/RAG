...
import { getExportUrl, getDocContent, exportFilesCsvOffline } from '../../api/fileApi';
            ...
            {(['document_analysis', 'correspondence'] as const).map((mode) => {
              ...
              return (
                ...
                >
                  <span
                    aria-hidden="true"
                    className={`w-1.5 h-1.5 ${isActive ? 'bg-[var(--accent)]' : 'bg-[var(--text-muted)]'}`}
                  />
          ...
          <div className="px-4 py-2 space-y-1 shrink-0 border-t border-[var(--border)]">
            {uploading.map((u) => (
              <div key={u.name} className="flex items-center gap-2 text-[10px]">
                ...
                <span className={u.status === 'completed' ? 'text-[var(--green)]' : u.status === 'error' ? 'text-[var(--danger)]' : 'text-[var(--text-muted)]'}>
        ...
        <div className="px-3 py-2 border-t border-[var(--border)] shrink-0 flex items-center justify-between gap-2">
          ...
          {files.length > 0 && (
            <a
              ...
              download
              onClick={(e) => {
                if (exportFilesCsvOffline(files)) e.preventDefault();
              }}
