...
# ── Schema-mapping confirmation loop (learning layer) ────────

class SchemaConfirmRequest(BaseModel):
    # Either a header-set → schema decision, or a per-column concept override.
...
@router.get("/admin/schema/pending")
def schema_pending(corpus: Optional[str] = None) -> dict:
    """Tables whose schema mapping is low-confidence and awaits confirmation."""
    from src.learning import schema_memory
    pending = schema_memory.pending_confirmations(corpus)
    return {"ok": True, "count": len(pending), "pending": pending}


@router.post("/admin/schema/confirm")
def schema_confirm(req: SchemaConfirmRequest) -> dict:
    """Record a human-confirmed mapping. Header-set→schema (columns+schema_id)
    persists so identical future uploads resolve identically; per-column
    override (table_id+raw_column+canonical_concept) corrects one table."""
    from src.learning import schema_memory
    if req.table_id and req.raw_column and req.canonical_concept:
        return schema_memory.confirm_column(
            req.table_id, req.raw_column, req.canonical_concept)
    if req.columns and req.schema_id:
        return schema_memory.confirm_schema_mapping(
            req.columns, req.schema_id, req.column_map)
    return {"ok": False, "reason": "provide (columns, schema_id) or "
            "(table_id, raw_column, canonical_concept)"}


@router.get("/admin/workflow/telemetry")
def workflow_telemetry(days: int = 30, corpus: str = "", recent: int = 50) -> dict:
    """Workflow-run aggregate + recent runs for the learning/observability panel."""
    from src.learning import telemetry_summary, recent_runs
    stats = telemetry_summary(corpus=corpus, days=days)
    stats["ok"] = True
    stats["recent"] = recent_runs(corpus=corpus, limit=recent)
    return stats


# ── Delay-event candidate analyst gate (confirm/reject/merge) ─

class EventDecisionRequest(BaseModel):
    candidate_id: str
    action: str                      # "confirm" | "reject"
    decided_by: str = ""
    reason: str = ""                 # required for "reject" (audit trail)
...
@router.get("/admin/delay-events/candidates")
def delay_event_candidates(corpus: str = "", status: Optional[str] = None,
                           project_id: Optional[str] = None) -> dict:
    """Candidate delay events awaiting (or past) analyst decision."""
    from src.delay_reports import candidate_store
    rows = candidate_store.list_candidates(corpus=corpus, status=status,
                                           project_id=project_id)
    return {"ok": True, "count": len(rows), "candidates": rows}


@router.get("/admin/delay-events/confirmed")
def delay_event_confirmed(corpus: str = "",
                          project_id: Optional[str] = None) -> dict:
    """Analyst-confirmed delay events (the only ones fit for claim use)."""
    from src.delay_reports import candidate_store
    rows = candidate_store.confirmed_events(corpus=corpus, project_id=project_id)
    return {"ok": True, "count": len(rows), "confirmed": rows}


@router.get("/admin/delay-events/{event_id}/evidence")
def delay_event_evidence(event_id: str) -> dict:
    """Source evidence (file/page/quote) backing one candidate/confirmed event."""
    from src.delay_reports import candidate_store
    ev = candidate_store.get_candidate(event_id)
    if ev is None:
        return {"ok": False, "reason": "unknown event_id"}
    return {"ok": True, "evidence": {
        "document": ev.get("file_name"), "doc_id": ev.get("doc_id"),
        "page": ev.get("page_number"), "date": ev.get("event_date"),
        "party": ev.get("actor"), "snippet": ev.get("quote"),
        "support": ev.get("support_level"), "status": ev.get("status"),
        "citation": f"{ev.get('file_name', '')}, p.{ev.get('page_number', '')}"}}


@router.post("/admin/delay-events/decide")
def delay_event_decide(req: EventDecisionRequest) -> dict:
    """Confirm or reject one candidate delay event. Rejection requires a reason
    (audit trail; nothing is hard-deleted)."""
    from src.delay_reports import candidate_store
    if req.action == "confirm":
        return candidate_store.set_status(
            req.candidate_id, candidate_store.STATUS_CONFIRMED, req.decided_by)
    if req.action == "reject":
        return candidate_store.set_status(
            req.candidate_id, candidate_store.STATUS_REJECTED, req.decided_by,
            reason=req.reason)
    return {"ok": False, "reason": "action must be 'confirm' or 'reject'"}


@router.post("/admin/delay-events/merge")
def delay_event_merge(req: EventMergeRequest) -> dict:
    """Merge duplicate candidate delay events into a primary (audit-preserving)."""
    from src.delay_reports import candidate_store
    return candidate_store.merge(req.primary_id, req.other_ids, req.decided_by)
